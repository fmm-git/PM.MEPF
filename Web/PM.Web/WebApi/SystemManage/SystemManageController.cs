    ...
    public class SystemManageController : ApiController
    {
        ...
        public HttpResponseMessage GetProjectInfoList(string url)
        {
            try
            {
                ...
                DataColumn dc = new DataColumn("InsertTime", typeof(DateTime));
                dc.DefaultValue = DateTime.Now;
                dt.Columns.Add(dc);
                List<TbProjectInfo> model = ModelConvertHelper<TbProjectInfo>.ToList(dt);
        ...
        public HttpResponseMessage GetCompanyList(string url)
        {
            try
            {
                ...
                dc.DefaultValue = 0;
                dt.Columns.Add(dc);
                DataColumn dc1 = new DataColumn("PostalCode", typeof(string));
                dc1.DefaultValue = "0";
                dt.Columns.Add(dc1);
