...
namespace PM.Business.EarlyWarning
{
    public class EarlyWarningTimer
    {
        ...
        public EarlyWarningTimer()
        {
            ...
            DateTime dt2 = Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM" + "-20 09:00:00"));
        ...
        public void GetEarlyInfo()
        {
            try
            {
                ...
                string sql = @"select TbEarlyInfo.*,DATEDIFF(MINUTE,DATEADD(DAY,TbSj.CsTriggerTimeDay,DATEADD(HOUR,TbSj.CsTriggerTimeHour,DATEADD(MINUTE,TbSj.CsTriggerTimeMinute,TbEarlyInfo.EarlyCsBegTiem))),GETDATE()) as SjJgMinute,cast(DATEDIFF(MINUTE,DATEADD(DAY,TbSj.CsTriggerTimeDay,DATEADD(HOUR,TbSj.CsTriggerTimeHour,DATEADD(MINUTE,TbSj.CsTriggerTimeMinute,TbEarlyInfo.EarlyCsBegTiem))),GETDATE())*1.0/60/24 as decimal(18,2)) as SjJgDay,cast(DATEDIFF(MINUTE,DATEADD(DAY,TbSj.CsTriggerTimeDay,DATEADD(HOUR,TbSj.CsTriggerTimeHour,DATEADD(MINUTE,TbSj.CsTriggerTimeMinute,TbEarlyInfo.EarlyCsBegTiem))),GETDATE())*1.0/60 as decimal(18,2)) as SjJgHour,TbFormEarlyInfo.ID as FormEarlyInfoID from (select TbEarlySz.ID,TbEarlySz.EarlyWarningNewsCode,TbEarlySz.EarlyWarningContent,TbEarlySz.MenuCode,TbEarlySz.EarlyWarningNewsName,TbEarlySz.App,TbEarlySz.Pc,TbEarlySz.IsStart,TbEarlySz.EarlyWarningFrequency,TbEarlySz.IsBackstage,TbEarlySz.EarlyMenuCodeNode,TbEarlySz.EarlyWarningFrequencyType,TbEarlySz.OrgType,TbEarlySz.PersonnelSource,TbEarlySz.PersonnelCode,TbEarlySz.DeptId,TbEarlySz.RoleId,TbEarlyMenu.ID as EarlyDataId,TbEarlyMenu.EarlyCode,TbEarlyMenu.ProjectId,TbEarlyMenu.EarlyTiem,
case when TbEarlySz.EarlyWarningFrequencyType='天' then DATEADD(DAY,TbEarlySz.EarlyWarningFrequency,TbEarlyMenu.EarlyTiem) 
when TbEarlySz.EarlyWarningFrequencyType='时' then DATEADD(HOUR,TbEarlySz.EarlyWarningFrequency,TbEarlyMenu.EarlyTiem)
else DATEADD(MINUTE,TbEarlySz.EarlyWarningFrequency,TbEarlyMenu.EarlyTiem) end as EarlyCsBegTiem,case when TbEarlySz.EarlyWarningFrequencyType='天' then DATEADD(DAY,TbEarlySz.EarlyWarningFrequency,DATEADD(DAY,TbEarlySz.TriggerTimeDay,DATEADD(HOUR,TbEarlySz.TriggerTimeHour,DATEADD(MINUTE,TbEarlySz.TriggerTimeMinute,TbEarlyMenu.EarlyTiem)))) when TbEarlySz.EarlyWarningFrequencyType='时' then DATEADD(HOUR,TbEarlySz.EarlyWarningFrequency,DATEADD(DAY,TbEarlySz.TriggerTimeDay,DATEADD(HOUR,TbEarlySz.TriggerTimeHour,DATEADD(MINUTE,TbEarlySz.TriggerTimeMinute,TbEarlyMenu.EarlyTiem))))else DATEADD(MINUTE,TbEarlySz.EarlyWarningFrequency,DATEADD(DAY,TbEarlySz.TriggerTimeDay,DATEADD(HOUR,TbEarlySz.TriggerTimeHour,DATEADD(MINUTE,TbEarlySz.TriggerTimeMinute,TbEarlyMenu.EarlyTiem)))) end as EarlyBegTiem,TbEarlyMenu.SiteCode,TbEarlyMenu.SiteName,TbEarlyMenu.WorkAreaCode,TbEarlyMenu.WorkAreaName,TbEarlyMenu.BranchCode,TbEarlyMenu.BranchName,TbEarlyMenu.ManagerDepartmentCode,TbEarlyMenu.ProcessFactoryCode,TbEarlyMenu.TypeCode,TbEarlyMenu.SiteContacts,TbEarlyMenu.SiteContactTel from (select a.*,b.OrgType,b.ProjectId,b.PersonnelSource,b.PersonnelCode,b.DeptId,b.RoleId from TbEarlyWarningSetUp a
                           ...
                           left join (select * from (select EWFormDataCode as ID,MenuCode,EarlyWarningCode,EWFormDataCode from TbFormEarlyWarningNodeInfo
						   union all
						   select EWFormDataCode as ID,FromCode,EarlyWarningCode,EWFormDataCode from TbSMSAlert) Tb group by ID,MenuCode,EarlyWarningCode,EWFormDataCode) TbFormEarlyInfo on TbEarlyInfo.MenuCode=TbFormEarlyInfo.MenuCode 
                           ...
                           left join (SELECT MenuCode as CsMenuCode,TriggerTimeDay as CsTriggerTimeDay,TriggerTimeHour as CsTriggerTimeHour,TriggerTimeMinute as CsTriggerTimeMinute FROM TbEarlyWarningSetUp A
                           WHERE NOT EXISTS (SELECT 1 FROM TbEarlyWarningSetUp WHERE MenuCode=A.MenuCode AND ID<A.ID)) TbSj on TbSj.CsMenuCode=TbEarlyInfo.MenuCode
                                                      where TbEarlyInfo.EarlyDataId is not null 
                           and GETDATE()>TbEarlyInfo.EarlyBegTiem and TbFormEarlyInfo.ID is null and TbEarlyInfo.EarlyBegTiem>'2020-04-01' ";
...
left join(select 'RawMonthDemandPlan' as MenuCode,TbGq.* from (select cp2.CompanyCode as WorkAreaCode,cp2.CompanyFullName as WorkAreaName,cp3.CompanyCode as BranchCode,cp3.CompanyFullName as BranchName,cp3.ParentCompanyCode as ManagerDepartmentCode,wo.ProjectId,wo.ProcessFactoryCode  from  TbWorkOrder wo
...
group by cp2.CompanyCode,cp2.CompanyFullName,cp3.CompanyCode,cp3.CompanyFullName,cp3.ParentCompanyCode,wo.ProjectId,wo.ProcessFactoryCode
union all
select rmp.WorkAreaCode,cp1.CompanyFullName as WorkAreaName,rmp.BranchCode,cp2.CompanyFullName as BranchName,cp2.ParentCompanyCode as ManagerDepartmentCode,rmp.ProjectId,rmp.ProcessFactoryCode from TbRawMaterialMonthDemandPlan rmp
...
where TbGq.WorkAreaCode not in(select WorkAreaCode from TbRawMaterialMonthDemandPlan rmp where rmp.DemandMonth=convert(varchar(7),dateadd(MONTH,1,getdate()),120)  and rmp.ProjectId=TbGq.ProjectId)
group by TbGq.ManagerDepartmentCode,TbGq.BranchCode,TbGq.BranchName,TbGq.WorkAreaCode,TbGq.WorkAreaName,TbGq.ProjectId,TbGq.ProcessFactoryCode) TbEarlyInfo on TbEarlySz.MenuCode=TbEarlyInfo.MenuCode and TbEarlySz.ProjectId=TbEarlyInfo.ProjectId) TbEarly
...
and TbEarly.EarlyDataId=TbFormEarlyInfo.EWFormDataCode and TbEarly.EarlyCode=TbFormEarlyInfo.DataCode) as TbEarlyInfo
where  GETDATE()>TbEarlyInfo.EarlyBegTiem and TbEarlyInfo.FormEarlyID is null ";
                DataTable dt = Db.Context.FromSql(sql).ToDataTable();
                if (dt.Rows.Count > 0)
                {
                    //查找消息模板信息
                    var shortMessageTemplateModel = Repository<TbShortMessageTemplate>.First(p => p.TemplateCode == "MB0001");
                    if (shortMessageTemplateModel != null)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
        ...
        public void GetTransportShortMessageWarn()
        {
            try
            {
                ...
                List<TbFormEarlyWarningNodeInfo> myMsgList = new List<TbFormEarlyWarningNodeInfo>();//PC端推送
                var NoticeModel = Repository<TbNoticeNewsSetUp>.First(p => p.NoticeNewsCode == "XXTZ0009" && p.IsStart == 1);
                if (NoticeModel != null)
                {
                    //查找消息模板信息
                    var shortMessageTemplateModel = Repository<TbShortMessageTemplate>.First(p => p.TemplateCode == "MB0009");
                    if (shortMessageTemplateModel != null)
                    {
