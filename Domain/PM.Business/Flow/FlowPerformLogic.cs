...
namespace PM.Business
{
    public class FlowPerformLogic
    {
        ...
        /// <returns></returns>
        public PageModel GetMyApproval(PageSearchRequest pt, string UserId, string ProjectId, int PerformState, DateTime? SDT, DateTime? EDT)
        {
            string where = "";
            if (!string.IsNullOrWhiteSpace(UserId))
            {
                where += " and o.UserCode=@UserId";
            }
            if (!string.IsNullOrWhiteSpace(ProjectId))
            {
                where += " and fd.ProjectId=@ProjectId";
            }
            if (PerformState == 111)//全部
            {
                where += " and o.PerformState!=-1 and o.PerformState!=7";
            }
            else
            {
                if (PerformState == -1)//待办
                {
                    where += " and (o.PerformState=-1 or o.PerformState=7)";
                }
                else
                {
                    where += " and o.PerformState=@PerformState";
                }
            }
            string sql = @"select o.PreNodeCompleteDate,o.FlowNodeCode,o.FlowPerformID,f.FlowCode,f.FormCode,f.MenuName,f.UserName,f.FormDataCode,o.UserType,
	fd.FlowName,
...
where a.ID=@ID";
                        ...
                        if (dtTableData.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < flowNodePersonnelList.Count; i++)
                            {
                                ...
                                {
                                    string OrgId = "";
                                    string ProjectId = flowNodePersonnelList[i].ProjectId;
                                    ...
                                    sqlUser = "select a.UserCode from TbUserRole a where 1=1 and a.Flag=0 and a.DeptId='" + flowNodePersonnelList[i].DeptId + "' and a.RoleCode='" + flowNodePersonnelList[i].RoleId + "' and a.ProjectId='" + ProjectId + "' and a.OrgType=" + flowNodePersonnelList[i].OrgType + " and a.OrgId='" + OrgId + "' ";
