...
where a.InOrderCode=@InOrderCode";
        ...
        public PageModel GetDataListForPage(InOrderRequest request)
        {
            ...
            {
                where += " and ino.RebarType=" + request.RebarType + "";
            }
            ...
            //Img1
            if (!string.IsNullOrWhiteSpace(request.ImgMonth))
            {
                int Year = DateTime.Now.Year;
                string Month = request.ImgMonth;
                if (request.ImgMonth.Length == 1)
                {
                    Month = "0" + request.ImgMonth;
                }
                string YearMonth = Year + "-" + Month;
                where += " and CONVERT(varchar(7), ino.InsertTime, 23)='" + YearMonth + "'";
            }
            //Img2
            if (!string.IsNullOrWhiteSpace(request.InType))
            {
                if (request.InType == "未取样")
                {
                    where += " and ino.SampleOrderState='取样未完成'";
                }
                else if (request.InType == "验收不合格")
                {
                    where += " and ino.NoPassTotal>0";
                }
                else if (request.InType == "检测部分合格")
                {
                    where += " and ino.ChackResult=3";
                }
                else if (request.InType == "检测不合格")
                {
                    where += " and  ino.ChackResult=2";
                }
            }
            //Img3
            if (!string.IsNullOrWhiteSpace(request.GhType))
            {
                if (request.GhType == "验收不合格")
                {
                    where += " and ino.NoPassTotal>0";
                }
                else
                {
                    where += " and (ino.ChackResult=3 or ino.ChackResult=2)";
                }
            }
            #endregion
...
and a.ProjectId='" + ProjectId + "' and (a.OrgId='" + BranchCode + "' or a.OrgId='" + WorkAreaCode + "') and a.Flag=0 and u.UserCode is not null";
            ...
            try
            {
                ...
                if (NoticeModel != null)
                {
                    ...
                    if (shortMessageTemplateModel != null)
                    {
                        ...
                        {
                            var batchPlanModel = Repository<TbFactoryBatchNeedPlan>.First(p => p.BatchPlanNum == inOrderModel.BatchPlanCode);
                            //获取用户userId
                                ...
                                if (dt.Rows.Count > 0)
                                {
                                    ...
                                    if (NoticeModel.Pc == 1)
                                    {
                                        var myFormEarlyMsg = new TbFormEarlyWarningNodeInfo()
                                        {
                                            ...
                                            EWStart = 0,
                                            EWTime = DateTime.Now,
                                            ProcessFactoryCode = batchPlanModel.ProcessFactoryCode,
                                            DataCode = inOrderModel.InOrderCode,
...
where a.InOrderCode='" + inOrderCode + "' and b.ChackState=3";
        ...
        public DataTable Img1(InOrderRequest request)
        {
            ...
            string sql = @"select Tb.DataMonth,ISNULL(sum(TbData.InorderCount),0) as InCount,ISNULL(sum(TbData.InCount),0) as InWeight from (select datepart(MONTH,dateadd(mm,-t.number,getdate())) as DataMonth
from
