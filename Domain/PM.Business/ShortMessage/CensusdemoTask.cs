    ...
    public class CensusdemoTask
    {
        ...
        public List<NotiecUser> GetSendUser(string FormCode, string NoticeNewsCode, int ID, string EarlyType = "")
        {
            ...
            if (menuTable != null)
            {
                ...
                {
                    sqlTableData = @"select a.ProjectId,a.BranchCode,a.WorkAreaCode,a.ProcessFactoryCode,b.ParentCompanyCode as JlbCode from " + menuTable.TableName + @" a
left join TbCompany b on a.BranchCode=b.CompanyCode where a.ID=@ID";
                }
                else if (FormCode == "InOrder")
                {
                    sqlTableData = @"select a.ProjectId,a.WorkAreaCode,b.ParentCompanyCode as BranchCode,c.ParentCompanyCode as JlbCode,d.ProcessFactoryCode from " + menuTable.TableName + @" a
...
left join TbStorage d on a.StorageCode=d.StorageCode  where a.ID=@ID";
                ...
                {
                    sqlTableData = @"select a.ProjectId,a.WorkAreaCode,b.ParentCompanyCode as BranchCode,c.ParentCompanyCode as JlbCode,a.ProcessFactoryCode from " + menuTable.TableName + @" a
left join TbCompany b on a.WorkAreaCode=b.CompanyCode
left join TbCompany c on b.ParentCompanyCode=c.CompanyCode  where a.ID=@ID";
                ...
                {
                    sqlTableData = @"select a.ProjectId,b.ParentCompanyCode as WorkAreaCode,c.ParentCompanyCode as BranchCode,d.ParentCompanyCode as JlbCode,a.ProcessFactoryCode from " + menuTable.TableName + @" a
...
where a.ID=@ID";
                ...
                {
                    string ProjectId=dtTableData.Rows[0]["ProjectId"].ToString();
                    if (!string.IsNullOrWhiteSpace(EarlyType))
                    {
                        var NoticeUserList1 = Repository<TbFlowNodeEarlyWarningPersonnel>.Query(p => p.EarlyWarningCode == NoticeNewsCode&&p.ProjectId==ProjectId);
                        string sqlUser = "";
                    ...
                    {
                        var NoticeUserList2 = Repository<TbNoticeNewsOrg>.Query(p => p.NoticeNewsCode == NoticeNewsCode&&p.ProjectId==ProjectId);
                        string sqlUser = "";
