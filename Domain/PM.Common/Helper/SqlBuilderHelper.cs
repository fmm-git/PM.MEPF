...
namespace PM.Common.Helper
{
    ...
    public class SqlBuilderHelper
    {
        ...
        public static string BulkInsertSql<T>(List<T> objs, string tableName) where T : class
        {
            ...
            string columns2 = GetColmons(objs[0],"t");
            if (string.IsNullOrEmpty(columns))
            {
                return string.Empty;
            }
            ...
            sql.Append("Insert into " + tableName);
            sql.Append("(" + columns + ")");
            sql.Append(" select " + columns2 + " from ");
            sql.Append("  (values " + values + ")");
            sql.Append(" as t(" + columns + ")");
            return sql.ToString();
        }
        ...
        /// <returns></returns>
        private static string GetColmons<T>(T obj,string table="")
        {
            if (obj == null)
            {
                return string.Empty;
            ...
            {
                return string.Join(",t.", obj.GetType().GetProperties().Select(p => p.Name).ToList());
            }
        }
